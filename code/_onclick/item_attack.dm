...
/atom/movable/attackby(obj/item/W, mob/living/user, params)
	...
	if(!(W.flags&NOBLUDGEON))
		visible_message("<span class='danger'>[src] foi golpeado por [user] com [W].</span>")
...
/obj/item/proc/attack(mob/living/M as mob, mob/living/user as mob, def_zone)
	...
	if(!istype(M, /mob/living/carbon/human))
		if(istype(M, /mob/living/carbon/slime))
			...
			if(prob(25))
				to_chat(user, "<span class='warning'>[src] passa diretamente [M]!</span>")
				return
		...
		for(var/mob/O in viewers(messagesource, null))
			if(attack_verb.len)
				O.show_message("<span class='combat danger'>[M] foi [pick(attack_verb)] com [src][showname] </span>", 1)
			else
				O.show_message("<span class='combat danger'>[M] foi atacado com [src][showname] </span>", 1)

		if(!showname && user)
			if(user.client)
				to_chat(user, "<span class='combat danger'>Você atacou [M] com [src]. </span>")
	...
	if(istype(M, /mob/living/carbon/human))
		...
		switch(damtype)
			...
			if("fire")
				...
				to_chat(M, "AI, isso queima(Tá pegando fogo bixo!)!")
		M.updatehealth()
