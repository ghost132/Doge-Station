...
/world/New()
	//logs
	var/date_string = time2text(world.realtime, "DD-Day/MM-Month/YYYY")
...
/world/Topic(T, addr, master, key)
	diary << "TOPIC: \"[T]\", De:[addr], master:[master], key:[key]"
	...
	if("ping" in input)
		...
		s["votar"] = config.allow_vote_mode
		...
		for(var/client/C in clients)
			...
			s["jogando[player_count]"] = C.key
			player_count++
		s["jogando"] = player_count
		s["admins"] = admin_count
		s["map_name"] = map_name ? map_name : "Desconhecido"
		...
		*/
		...
		return show_player_info_irc(input["nota"])

	else if("anuncio" in input)
...
/world/Reboot(var/reason, var/feedback_c, var/feedback_r, var/time)
	if(reason == 1) //special reboot, do none of the normal stuff
		...
		spawn(0)
			to_chat(world, "<span class='boldannounce'>Pedido de reinicio do mundo imediato feito pelo admin</span>")
	...
	if(ticker.delay_end)
		to_chat(world, "<span class='boldannounce'>Um admin prorrogou o round.</span>")
		return
	to_chat(world, "<span class='boldannounce'>Reiniciando o mundo em [delay/10] [delay > 10 ? "seconds" : "second"]. [reason]</span>")
	...
	if(ticker.delay_end)
		to_chat(world, "<span class='boldannounce'>Reinicio cancelado pelo admin.</span>")
		return
	feedback_set_details("[feedback_c]","[feedback_r]")
	log_game("<span class='boldannounce'>Reiniciando o mundo. [reason]</span>")
...
/world/proc/update_status()
	...
	*/
	if(!host && config && config.hostedby)
		features += "Oferecido por <b>[config.hostedby]</b>"
...
/hook/startup/proc/connectDB()
	if(!setup_database_connection())
		log_to_dd("Seu sever falhou ao se conectar ao banco de dados.")
	else
		log_to_dd("Feedback com o banco de dados estabelecido.")
	return 1
