...
/obj/machinery/media/jukebox/attack_hand(var/mob/user)
	...
	t += "<b>Power:</b> <a href='?src=[UID()];power=1'>[playing?"On":"Off"]</a><br />"
	t += "<b>Play Mode:</b> <a href='?src=[UID()];mode=1'>[loopModeNames[loop_mode]]</a><br />"
	if(playlist == null)
		...
		if(check_reload())
			...
			for(var/plid in playlists)
				t += "<a href='?src=[UID()];playlist=[plid]'>[playlists[plid]]</a>"
		else
		...
		for(i = 1,i <= playlist.len,i++)
			...
			t += "<tr><th>#[i]</th><td><A href='?src=[UID()];song=[i]' class='nobg'>[song.displaytitle()]</A></td><td>[song.album]</td></tr>"
		t += "</table>"
