...
/obj/machinery/media/transmitter/broadcast/multitool_menu(var/mob/user,var/obj/item/device/multitool/P)
	...
	<h2>Settings</h2>
	<ul>
		<li><b>Power:</b> <a href="?src=[UID()];power=1">[on?"On":"Off"]</a></li>
		<li><b>Frequency:</b> <a href="?src=[UID()];set_freq=-1">[format_frequency(media_frequency)] GHz</a> (<a href="?src=[UID()];set_freq=[initial(media_frequency)]">Reset</a>)</li>
	</ul>
	<h2>Media Sources</h2>"}
	if(!sources.len)
		...
		for(var/i=1;i<=sources.len;i++)
			...
			screen += "<li>\ref[source] [source.name] ([source.id_tag])  <a href='?src=[UID()];unlink=[i]'>\[X\]</a></li>"
		screen += "</ol>"
	return screen
*/
