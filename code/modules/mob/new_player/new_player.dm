...
/mob/new_player/proc/new_player_panel_proc()
	...
	if(client.prefs.randomslot)
		real_name = "Slot de Personagem Aleatorio"
	var/output = "<center><p><a href='byond://?src=[UID()];show_preferences=1'>Configurar personagem</A><br /><i>[real_name]</i></p>"

	if(!ticker || ticker.current_state <= GAME_STATE_PREGAME)
		if(!ready)	output += "<p><a href='byond://?src=[UID()];ready=1'>Pronto!</A></p>"
		else	output += "<p><b>You are ready</b> (<a href='byond://?src=[UID()];ready=2'>Cancelar</A>)</p>"

	else
		output += "<p><a href='byond://?src=[UID()];manifest=1'>Ver lista de funcionarios</A></p>"
		output += "<p><a href='byond://?src=[UID()];late_join=1'>Juntar-se ao jogo!</A></p>"
	...
	var/datum/browser/popup = new(src, "playersetup", "<div align='center'>Opções de novo jogador</div>", 220, 290)
...
/mob/new_player/Topic(href, href_list[])
	...
	if(href_list["observe"])
		...
		if(alert(src,"Tem certeza quer so observar? Você nao poderá voltar a partidade depois que fizer isso!","Configuração do Jogador","Sim","Não") == "Sim")
			...
			to_chat(src, "<span class='notice'>Teleportando.</span>")
			observer.loc = O.loc
	...
	if(href_list["late_join"])
		if(!ticker || ticker.current_state != GAME_STATE_PLAYING)
			to_chat(usr, "<span class='warning'>O round ainda não está pronto, ou acabou de acabar...</span>")
			return
