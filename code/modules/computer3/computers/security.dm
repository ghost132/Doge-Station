...
/datum/file/program/secure_data
	...
	interact()
		...
		if (temp)
			...
			dat = "Confirm Identity (R): <A href='?src=[UID()];choice=Confirm Identity R'>[scan ? scan.name : "----------"]</A><HR>"
			if (computer.cardslot.dualslot)
				dat += "Check Identity (W): <A href='?src=[UID()];choice=Confirm Identity W'>[scan2 ? scan2.name : "----------"]</A><BR>"
				if(scan2 && !scan)
					dat += "<div class='notice'>Insert card into reader slot to log in.</div><br>"
			if (authenticated)
				switch(screen)
					if(1.0)
						dat += {"
	...
	<th>Criminal Status</th>
					...
					if(3.0)
						...
						if ((istype(active2, /datum/data/record) && data_core.security.Find(active2)))
							...
							<BR>\nCriminal Status: <A href='?src=[UID()];choice=Edit Field;field=criminal'>[active2.fields["criminal"]]</A><BR>\n
							...
							<BR>\nImportant Notes:<BR>\n\t<A href='?src=[UID()];choice=Edit Field;field=notes'>[decode(active2.fields["notes"])]</A><BR>\n
