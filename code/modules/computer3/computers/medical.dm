...
/datum/file/program/med_data
	...
	interact()
		...
		if (temp)
			dat = "<TT>[temp]</TT><BR><BR><A href='?src=[UID()];temp=1'>Clear Screen</A>"
		else
			dat = "Confirm Identity (R): <A href='?src=[UID()];cardr=1'>[scan ? scan.name : "----------"]</A><HR>"
			if (computer.cardslot.dualslot)
				dat += "Check Identity (W): <A href='?src=[UID()];cardw=1'>[scan2 ? scan2.name : "----------"]</A><BR>"
				if(scan2 && !scan)
...
"}
					if(2.0)
						...
						if(!isnull(data_core.general))
							for(var/datum/data/record/R in sortRecord(data_core.general))
								dat += "<A href='?src=[UID()];d_rec=\ref[R]'>[R.fields["id"]]: [R.fields["name"]]<BR>"
								//Foreach goto(132)
					...
					if(4.0)
						...
						if ((istype(src.active2, /datum/data/record) && data_core.medical.Find(src.active2)))
							...
							<BR>\nImportant Notes:<BR>\n\t<A href='?src=[UID()];field=notes'>[decode(active2.fields["notes"])]</A><BR>\n
