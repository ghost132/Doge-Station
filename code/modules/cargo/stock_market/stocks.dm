...
/datum/stock/proc/generateEvent(event_type)
	var/datum/stockEvent/E = new event_type(src)
	addEvent(E)
...
/datum/stock/proc/modifyAccount(whose, by, force=0)
	if(shuttle_master.points)
		if(by < 0 && shuttle_master.points + by < 0 && !force)
			return 0
		shuttle_master.points += by
