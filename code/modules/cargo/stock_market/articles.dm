...
/datum/article/proc/generateAuthorName()
	switch(rand(1,3))
		if(1)
			return "[uppertext(pick(consonants))]. [pick(last_names)]"
		if(2)
			return "[prob(50) ? pick(first_names_male) : pick(first_names_female)] [uppertext(pick(consonants))].[prob(50) ? "[uppertext(pick(consonants))]. " : null] [pick(last_names)]"
		if(3)
