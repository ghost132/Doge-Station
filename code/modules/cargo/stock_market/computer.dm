...
a.updated {
	...
	for(var/datum/stock/S in stockExchange.last_read)
		...
		if(!(logged_in in LR))
			LR[logged_in] = 0
	dat += "<b>View mode:</b> <a href='?src=[UID()];cycleview=1'>[vmode ? "Compact" : "Full"]</a> "
	dat += "<b>Stock Transaction Log:</b> <a href='?src=[UID()];show_logs=1'>Check</a><br>"
	...
	if(vmode == 0)
		for(var/datum/stock/S in stockExchange.stocks)
			...
			dat += "<b>Current value per share:</b> [S.current_value] | <a href='?src=[UID()];viewhistory=\ref[S]'>View history</a><br><br>"
			...
			if(S.bankrupt)
				...
				dat += "<a href='?src=[UID()];buyshares=\ref[S]'>Buy shares</a> | <a href='?src=[UID()];sellshares=\ref[S]'>Sell shares</a><br><br>"
				...
				if(!news)
					for(var/datum/stockEvent/E in S.events)
						if(E.last_change > lrt && !E.hidden)
							news = 1
							break
			dat += "<a href='?src=[UID()];archive=\ref[S]'>View news archives</a>[news ? " <span style='color:red'>(updated)</span>" : null]</div>"
	else if(vmode == 1)
		...
		for(var/datum/stock/S in stockExchange.stocks)
			...
			if(S.bankrupt)
				...
				dat += "<a href='?src=[UID()];buyshares=\ref[S]'>+</a> <a href='?src=[UID()];sellshares=\ref[S]'>-</a> "
			dat += "<a href='?src=[UID()];archive=\ref[S]' class='[news ? "updated" : "default"]'>(A)</a> <a href='?src=[UID()];viewhistory=\ref[S]'>(H)</a></td>"
...
/obj/machinery/computer/stockexchange/Topic(href, href_list)
	...
	if(href_list["show_logs"])
		var/dat = "<html><head><title>Stock Transaction Logs</title></head><body><h2>Stock Transaction Logs</h2><div><a href='?src=[UID()];show_logs=1'>Refresh</a></div><br>"
	...
	if(href_list["archive"])
		...
		var/dat = "<html><head><title>News feed for [S.name]</title></head><body><h2>News feed for [S.name]</h2><div><a href='?src=[UID()];archive=\ref[S]'>Refresh</a></div>"
