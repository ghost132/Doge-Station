/obj/item/weapon/cartridge
	name = "Cartucho generico"
	desc = "Um cartucho para micro computadores."
...
/obj/item/weapon/cartridge/signal
	name = "artucho generico signaler"
	desc = "Um cartucho de dados com um modulo sinalizador de radio."
...
/obj/item/weapon/cartridge/signal/toxins
	name = "Signal Ace 2"
	desc = "Completo com sinalizador de radio!"
	icon_state = "cart-tox"
	programs = list(
...
/obj/item/weapon/cartridge/quartermaster
	name = "CartuchoSpace Parts & Space Vendors"
	desc = "Perfeito para o superentendente!"
	icon_state = "cart-q"
	programs = list(
...
/obj/item/weapon/cartridge/mob_hunt_game
	name = "Cartucho Nano-Mob Hunter GO!(pokemon go espacial)"
	desc = "O novo jogo de PDA sucesso de vendas, capture os seus Nano-Mob favoritos pelo mundo!"
	icon_state = "cart-eye"
...
/obj/item/weapon/cartridge/mob_hunt_game/attackby(obj/item/O, mob/user, params)
	if(istype(O, /obj/item/weapon/nanomob_card))
		...
		if(my_game.register_capture(card.mob_data))
			to_chat(user, "<span class='notice'>Transferencia bem-sucedida!</span>")
			qdel(card)
		else
			to_chat(user, "<span class='warning'>Transferencia mal-sucedida. Não pôde ler os dados do mob.</span>")
	else
		..()
...
/obj/item/weapon/cartridge/mob_hunt_game/emag_act(mob/user)
	if(!emagged)
		...
		var/datum/data/pda/app/mob_hunter_game/my_game = programs[1]
		my_game.hacked = 1
		to_chat(user, "<span class='warning'>TR4P_M45T3R.mod inicializado com sucesso. ToS violado. User Agreement nulificado. Gotta pwn them all.</span>")
		to_chat(user, "<span class='warning'>Agora você pode criar versões presas de qualquer mob na sua coleção que irá prejudicar os caçadores que tentam capturá-la.</span>")
		description_antag = "Esta é uma copia de Nano-Mob Hunter GO! foi pirateado para permitir a criação de mobs de armadilha que causará qualquer PDA que tente capturar isso para dar um choque qualquer pessoa que a abrace. Copias hackeadas não vão ativar as armadilhas."
