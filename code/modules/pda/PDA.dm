...
/obj/item/device/pda/AltClick(mob/user)
	...
	if(can_use(user))
		if(id)
			...
			to_chat(user, "<span class='warning'>Este PDA nao tem um ID nele!</span>")
...
/obj/item/device/pda/attackby(obj/item/C as obj, mob/user as mob, params)
	...
	if(istype(C, /obj/item/weapon/cartridge) && !cartridge)
		...
		if(!idcard.registered_name)
			to_chat(user, "<span class='notice'>\the [src] Rejeitou o ID.</span>")
			return
		if(!owner)
		...
		C.forceMove(src)
		pai = C
		to_chat(user, "<span class='notice'>Seu slot \the [C] dentro [src].</span>")
...
/obj/item/device/pda/proc/explode() //This needs tuning.
	...
	if(ismob(loc))
		var/mob/M = loc
		M.show_message("<span class='danger'>Seu [src] EXPLODIU!</span>", 1)
...
/obj/item/device/pda/proc/set_ringtone()
	var/t = input("Por favor insira o novo toque", name, ttone) as text
	if(in_range(src, usr) && loc == usr)
		if(t)
			if(hidden_uplink && hidden_uplink.check_trigger(usr, lowertext(t), lowertext(lock_code)))
				to_chat(usr, "O PDA beepou suavemente.")
				close(usr)
			else
