...
/datum/chatroom
	var/name = "Sala de chat Generica"
	var/list/users = list()
...
/datum/chatroom/proc/login(datum/data/pda/app/chatroom/user)
	...
	users |= user
	announce(user, "[user.pda.owner] entrou #[name].")
	return 1
...
/datum/chatroom/proc/logout(datum/data/pda/app/chatroom/user)
	...
	users -= user
	invites -= user
	announce(user, "[user.pda.owner] saiu #[name].")
...
/datum/data/pda/app/chatroom/proc/check_messaging_available()
	. = messaging_available()
	if(!.)
		to_chat(usr, "<span class='notice'>ERROR: O servidor de menssagem não está respondendo.</span>")
	...
	switch(href_list["choice"])
		...
		if("New Room")
			...
			spawn()
				...
				for(var/datum/chatroom/C in chatrooms)
					if(C.name == t)
						alert("Não existe canal com esse nome.", name)
						return
				...
				if(!current_room.is_public)
					current_room.announce(src, "Os usúarios tem que ser convidados para entras na sala.")
		if("Autoscroll")
