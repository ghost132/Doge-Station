...
/obj/item/device/pda/silicon/verb/cmd_send_pdamesg()
	...
	if(!M)
		to_chat(usr, "<span class='warning'>Não pode usar o Mensseger!</span>")
...
/obj/item/device/pda/silicon/verb/cmd_show_message_log()
	set category = "AI IM"
	set name = "Mostrar log de menssagens"
	set src in usr
...
/obj/item/device/pda/silicon/verb/cmd_toggle_pda_silent()
	...
	to_chat(usr, "<span class='notice'>PDA Toque ativado [(M.notify_silent ? "Off" : "On")]!</span>")
...
/obj/item/device/pda/silicon/pai/can_use()
	...
	if(!pAI.software["messenger"])
		to_chat(usr, "<span class='warning'>Você não comprou um mensseger virtual!</span>")
		return 0
