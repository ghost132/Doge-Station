...
/datum/data/pda/app/notekeeper/start()
	. = ..()
	if(!note)
		note = "Parabéns, sua estação foi escolhida [pda.model_name]!"
...
/datum/data/pda/app/notekeeper/Topic(href, list/href_list)
	switch(href_list["choice"])
		if("Edit")
			var/n = input("Por favor insira sua menssagem", name, notehtml) as message
			if(pda.loc == usr)
				note = adminscrub(n)
