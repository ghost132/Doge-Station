...
/datum/data/pda/app/mob_hunter_game/proc/reconnect()
	...
	connected = 1
	if(pda)
		pda.audible_message("[bicon(pda)] Conexão estabelecida. Capture todos os mobs, [pda.owner ? pda.owner : "hunter"]!", null, 2)
	return 1
...
/datum/data/pda/app/mob_hunter_game/proc/disconnect(reason = null)
	...
	if(pda && reason)
		pda.audible_message("[bicon(pda)] Disconectou do servidor. Razão: [reason].", null, 2)
