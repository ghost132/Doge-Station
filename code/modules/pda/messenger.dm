...
/datum/data/pda/app/messenger/proc/create_message(var/mob/living/U, var/obj/item/device/pda/P)
	var/t = input(U, "Por favor insira a menssagem", name, null) as text|null
	if(!t)
		return
	...
	if(useMS && useTC) // only send the message if it's stable
		if(useTC != 2) // Does our recipient have a broadcaster on their level?
			to_chat(U, "ERROR: Nao pôde encontrar o recipiente.")
			return
		...
		to_chat(U, "<span class='notice'>ERROR: O servidor de menssagem não responde.</span>")
...
/datum/data/pda/app/messenger/proc/available_pdas()
	...
	if(toff)
		to_chat(usr, "Ligue o transmissor de menssagem primeiro.")
		return
