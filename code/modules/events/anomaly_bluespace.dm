...
/datum/event/anomaly/anomaly_bluespace/announce()
	event_announcement.Announce("Anomalia de bluespace instável detectada em scanners de longo alcance. Local esperado: [impact_area.name].", "Alerta de Anomalias")
	...
	if(newAnomaly)//If it hasn't been neutralized, it's time to warp half the station away jeez
		...
		if(T)
			...
			if(chosen)
				...
				event_announcement.Announce("A translocação de bluespace maciça detectada.", "Alerta de Anomalias")
