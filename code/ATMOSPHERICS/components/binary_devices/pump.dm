...
/obj/machinery/atmospherics/binary/pump/highcap
	name = "Bomba de gás de alta capacidade"
	desc = "Uma bomba de alta capacidade"
...
/obj/machinery/atmospherics/binary/pump/Topic(href,href_list)
	...
	if(href_list["power"])
		on = !on
		investigate_log("foi alternado para [on ? "on" : "off"] por [key_name(usr)]", "atmos")
		. = TRUE
	if(href_list["pressure"])
		...
		if(pressure == "max")
			...
			pressure = input("Nova pressão de saida inserida (0-[MAX_OUTPUT_PRESSURE] kPa):", name, target_pressure) as num|null
			if(!isnull(pressure))
				. = TRUE
		...
		if(.)
			...
			investigate_log("Alterado para [target_pressure] kPa por [key_name(usr)]", "atmos")
...
/obj/machinery/atmospherics/binary/pump/attackby(obj/item/weapon/W, mob/user, params)
	if(!istype(W, /obj/item/weapon/wrench))
		return ..()
	if(!(stat & NOPOWER) && on)
		to_chat(user, "<span class='alert'>Você nao pode desparafusar isso [src], Deslihue primeiro.</span>")
		return 1
	return ..()