/obj/machinery/atmospherics/binary/dp_vent_pump
	...
	name = "ventilador de dupla porta"
	desc = "Tem uma valvula e uma bomba acopladas a ela. Há duas portas."
...
/obj/machinery/atmospherics/binary/dp_vent_pump/high_volume
	name = "ventilador largo de duas portas "
...
/obj/machinery/atmospherics/binary/dp_vent_pump/update_icon(var/safety = 0)
	...
	overlays += icon_manager.get_atmos_icon("dispositivo", , , vent_icon)
...
/obj/machinery/atmospherics/binary/dp_vent_pump/proc/broadcast_status()
	...
	signal.data = list(
		...
		"dispositivo" = "DVB",
		"força" = on,
		"direção" = pump_direction?("soltar"):("sifão"),
		"checagem" = pressure_checks,
		"entrada" = input_pressure_min,
		"saida" = output_pressure_max,
		"externo" = external_pressure_bound,
		"sigtype" = "status"
	)
...
/obj/machinery/atmospherics/binary/dp_vent_pump/receive_signal(datum/signal/signal)
	...
	if(signal.data["alternar força"] != null)
		on = !on

	if(signal.data["direção"] != null)
		pump_direction = text2num(signal.data["direção"])

	if(signal.data["checagem"] != null)
		pressure_checks = text2num(signal.data["checks"])

	if(signal.data["expurgar"])
		pressure_checks &= ~1
		pump_direction = 0

	if(signal.data["estabilizar"])//the fact that this was "stabalize" shows how many fucks people give about these wonders, none
		pressure_checks |= 1
		pump_direction = 1

	if(signal.data["coloque a pressão de entrada"] != null)
		input_pressure_min = between(
			0,
			text2num(signal.data["coloque a pressão de entrada"]),
			ONE_ATMOSPHERE*50
		)

	if(signal.data["coloque a pressão de saida"] != null)
		output_pressure_max = between(
			0,
			text2num(signal.data["coloque a pressao de saida"]),
			ONE_ATMOSPHERE*50
		)

	if(signal.data["coloque a pressão externa"] != null)
		external_pressure_bound = between(
			0,
			text2num(signal.data["coloque a pressão externa"]),
			ONE_ATMOSPHERE*50
		)
