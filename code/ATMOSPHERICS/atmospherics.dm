...
/obj/machinery/atmospherics/attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
	if(can_unwrench && istype(W, /obj/item/weapon/wrench))
		...
		if(level == 1 && isturf(T) && T.intact)
			to_chat(user, "<span class='danger'>Você precisa remover a chaa primeiro.</span>")
			return 1
		...
		to_chat(user, "<span class='notice'>Você começou a desafivelar \the [src]...</span>")
		if(internal_pressure > 2*ONE_ATMOSPHERE)
			to_chat(user, "<span class='warning'>Você esta desparafusando \the [src] uma massa de ar sobprou em seu rosto... talvêz você deva reconsiderar?</span>")
			unsafe_wrenching = TRUE //Oh dear oh dear

		if(do_after(user, 40 * W.toolspeed, target = src) && isnull(gcDestroyed))
			user.visible_message( \
				"[user] unfastens \the [src].", \
				"<span class='notice'>Você esta sem cinto \the [src].</span>", \
				"<span class='italics'>Você ouviu a catraca.</span>")
...
/obj/machinery/atmospherics/proc/unsafe_pressure_release(var/mob/user,var/pressures)
	...
	user.visible_message("<span class='danger'>[user] foi mandado pelos ares pela pressão!</span>","<span class='userdanger'>A pressão te mandou pelos ares!</span>")
...
/obj/machinery/atmospherics/relaymove(mob/living/user, direction)
	...
	if(target_move)
		if(is_type_in_list(target_move, ventcrawl_machinery) && target_move.can_crawl_through())
			...
			user.visible_message("Você escutou um ruido pelos dutos.", "Você subiu pelo sistema de ventilação.")
		...
		if((direction & initialize_directions) || is_type_in_list(src, ventcrawl_machinery)) //if we move in a way the pipe can connect, but doesn't - or we're in a vent
			...
			user.visible_message("Você escutou um ruido nos canos.", "Você subiu pelo sistema de ventilação.")
	user.canmove = 0
	spawn(1)
		user.canmove = 1
