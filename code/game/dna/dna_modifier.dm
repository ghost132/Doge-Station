...
/obj/machinery/dna_scannernew/verb/eject()
	set src in oview(1)
	set category = null
	set name = "Ejetar Scanner de DNA"
...
/obj/machinery/dna_scannernew/verb/move_inside()
	set src in oview(1)
	set category = null
	set name = "Colocar no Scanner de DNA"
	...
	if(!ishuman(usr)) //Make sure they're a mob that has dna
		to_chat(usr, "<span class='notice'>Tentou o quanto pode, mas nao conseguiu subir em [src].</span>")
		return
	if(src.occupant)
		to_chat(usr, "<span class='boldnotice'>O [src] está ocupado!</span>")
		return
	if(usr.abiotic())
		to_chat(usr, "<span class='boldnotice'>O objeto não tem itens abiotico.</span>")
		return
	usr.stop_pulling()
	usr.forceMove(src)
	src.occupant = usr
...
/obj/machinery/dna_scannernew/MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
	...
	if(!istype(user.loc, /turf) || !istype(O.loc, /turf)) // are you in a container/closet/pod/etc?
		return
	if(occupant)
		to_chat(user, "<span class='boldnotice'>O [src] está ocupado!</span>")
		return
	var/mob/living/L = O
	...
	for(var/mob/living/carbon/slime/M in range(1,L))
		if(M.Victim == L)
			to_chat(usr, "[L.name] você não cabe dentro do [src] porque eles têm uma slime preso na cabeça.")
			return
	if(L == user)
...
/obj/machinery/dna_scannernew/attackby(var/obj/item/weapon/item as obj, var/mob/user as mob, params)
	if(istype(item, /obj/item/weapon/screwdriver))
		if(occupant)
			to_chat(user, "<span class='notice'>O painel de manutenção está travado.</span>")
			return
	...
	if(istype(item, /obj/item/weapon/crowbar))
		...
		if(!user.drop_item())
			to_chat(user, "<span class='warning'>\the [item] está preso em você!</span>")
			return

		beaker = item
		item.forceMove(src)
		user.visible_message("[user] adcione \a [item] a \the [src]!", "Você adcionou \a [item] a \the [src]!")
		return
	...
	if(!ismob(G.affecting))
		return
	if(src.occupant)
		to_chat(user, "<span class='boldnotice'>O scanner está ocupado!</span>")
		return
	if(G.affecting.abiotic())
		to_chat(user, "<span class='boldnotice'>O objeto nao tem itens abioticos nele.</span>")
		return
	if(panel_open)
...
/obj/machinery/dna_scannernew/proc/go_out()
	if(!src.occupant)
		to_chat(usr, "<span class='warning'>O scanner está vazio</span>")
		return

	if(src.locked)
		to_chat(usr, "<span class='warning'>O scanner está trancado!</span>")
		return
...
/obj/machinery/dna_scannernew/proc/radiation_check()
	...
	var/radiation_protection = occupant.run_armor_check(null, "rad", "Suas roupas estão quentes.", "Suas roupas estão quentes.")
...
/obj/machinery/computer/scan_consolenew
	name = "\improper Console de acesso ao modificador de DNA"
	desc = "Permite que você acesse e modifique o DNA."
