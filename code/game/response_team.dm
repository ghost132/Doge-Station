...
/client/proc/response_team()
	...
	if(!ticker)
		to_chat(usr, "<span class='warning'>O jogo ainda nao começou!</span>")
		return

	if(ticker.current_state == 1)
		to_chat(usr, "<span class='warning'>O round ainda nao começou!</span>")
		return
	...
	if(alert("Deseja despachar uma Equipe de Resposta à Emergencias?",,"Sim","Não") != "Sim")
		return

	if(get_security_level() != "red") // Allow admins to reconsider if the alert level isn't Red
		switch(alert("A estação nao está em alerta vermelho. Ainda quer dispachar uma euipe de resposta?",,"Sim","Não"))
			if("No")
				return
	...
	message_admins("[key_name_admin(usr)] despachou uma equipe de respostas à emergencias", 1)
...
/client/proc/pick_ert_type()
	switch(alert("Por favor escolha qual ERE deseja despachar.", "Equipe de Respostas à Emergencias", "Codigo Ambar", "Codigo Vermelho", "Codigo Gamma", "Cancelar"))
		if("Code Amber")
...
/mob/dead/observer/proc/JoinResponseTeam()
	...
	if(jobban_isbanned(src, ROLE_ERT))
		to_chat(src, "<span class='warning'>Você foi demitido da esuipe de resposta à emergencias!</span>")
		return 0

	var/player_age_check = check_client_age(client, responseteam_age)
	if(player_age_check && config.use_age_restriction_for_antags)
		to_chat(src, "<span class='warning'>Esse cargo não esta disponivel para você. Você precisa esperar por [player_age_check] dias.</span>")
		return 0
...
/proc/trigger_armed_response_team(var/datum/response_team/response_team_type)
	...
	var/list/ert_candidates = pollCandidates("Juntar-se a Equipe de Resposta à Emergencia?",, responseteam_age, 600, 1, role_playtime_requirements[ROLE_ERT])
...
/client/proc/create_response_team(var/turf/spawn_location)
	...
	var/new_gender = alert(src, "Por favor insira sua idade.", "Geração de Personagem ERE", "Hoem", "Mulher")
...
/datum/response_team/proc/get_slot_list()
	...
	if(security_slots)
		slots_available |= "Segurança"
	if(engineer_slots)
		slots_available |= "Engenheiro"
	if(medical_slots)
		slots_available |= "Médico"
...
/datum/response_team/proc/cannot_send_team()
	event_announcement.Announce("[station_name()], nos infelizmente não podemos mandar nenhuma Equipe de Resposta à Emergencias agora.", "ERE Indisponivel")

/datum/response_team/proc/announce_team()
	event_announcement.Announce("Attention, [station_name()]. Nós estamos enviando assistentes altamente treinados para ajudar você(?). Aguarde.", "ERE A caminho")
...
/datum/response_team/amber/announce_team()
	event_announcement.Announce("Attention, [station_name()]. Nós estamos enviando uma Equipe de Resposta à Emergencias de codigo AmBAR. Aguarde.", "ERE A caminho")
...
/datum/response_team/red/announce_team()
	event_announcement.Announce("Attention, [station_name()]. Nós estamos enviando uma Equipe de Resposta à Emergencias de codigo VERMELHO. Aguarde.", "ERE A caminho")
...
/datum/response_team/gamma/announce_team()
	event_announcement.Announce("Attention, [station_name()]. Nós estamos enviando uma Equipe de Resposta à Emergencias de elite de codigo GAMMA. Aguarde.", "ERE A caminho")
