...
/obj/item/examine(mob/user, var/distance = -1)
	...
	switch(src.w_class)
		...
		if(WEIGHT_CLASS_BULKY)
			size = "Um pouco grande"
		if(WEIGHT_CLASS_HUGE)
			size = "enorme"
		if(WEIGHT_CLASS_GIGANTIC)
			size = "gigante"

	. = ..(user, distance, "", "É um item de tamanho [size].")

	if(user.research_scanner) //Mob has a research scanner active.
		var/msg = "*--------* <BR>"

		if(origin_tech)
			msg += "<span class='notice'>Testando potenciais:</span><BR>"
			var/list/techlvls = params2list(origin_tech)
			for(var/T in techlvls) //This needs to use the better names.
				msg += "Tech: [CallTechName(T)] | Magnitude: [techlvls[T]] <BR>"
		else
			msg += "<span class='danger'>Nenhuma origem teconologica detectada.</span><BR>"
		...
		if(materials.len)
			...
			msg += "<span class='danger'>Nenhum material extrativo foi detectado.</span><BR>"
		msg += "*--------*"
		to_chat(user, msg)
...
/obj/item/attack_hand(mob/user as mob, pickupfireoverride = FALSE)
	...
	if(hasorgans(user))
		...
		if(!temp)
			to_chat(user, "<span class='warning'>Você tentou usar sua mão, mas ela esta desaparecida!</span>")
			return 0
		if(temp && !temp.is_usable())
			to_chat(user, "<span class='warning'>Você tentou mover [temp.name], mas nao pode!</span>")
			return 0
