...
/proc/explosion(turf/epicenter, devastation_range, heavy_impact_range, light_impact_range, flash_range, adminlog = 1, ignorecap = 0, flame_range = 0 ,silent = 0, smoke = 1)
	...
	spawn(0)
		...
		if(adminlog)
			message_admins("Explosão com o tamanho de ([devastation_range], [heavy_impact_range], [light_impact_range], [flame_range]) na área [epicenter.loc.name] ([epicenter.x],[epicenter.y],[epicenter.z]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[epicenter.x];Y=[epicenter.y];Z=[epicenter.z]'>JMP</a>)")
			log_game("Explosão com o tamanho de ([devastation_range], [heavy_impact_range], [light_impact_range], [flame_range]) na área [epicenter.loc.name] ([epicenter.x],[epicenter.y],[epicenter.z])")
...
*/
		...
		if(Debug2)	log_to_dd("## DEBUG: Explosão([x0],[y0],[z0])(d[devastation_range],h[heavy_impact_range],l[light_impact_range]): Levou [took] segundos.")
...
/client/proc/check_bomb_impacts()
	set name = "Checar impacto da bomba"
	set category = "Debug"

	var/newmode = alert("Usar bomba reacionaria?","Checar impacto da bomba", "Sim", "Não")
	...
	var/heavy = 0
	var/light = 0
	var/list/choices = list("Bomba pequena","Bomba media","Bomba grande","Bomba customizada")
	...
	switch(choice)
		...
		if("Bomba grande")
			...
			heavy = 5
			light = 7
		if("Bomba customizada")
			dev = input("Raio da devastação (Tiles):") as num
			heavy = input("Raio da devastação pesada (Tiles):") as num
			light = input("Raio da devastação leve (Tiles):") as num
